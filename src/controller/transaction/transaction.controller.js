...
module.exports = {
  getAllTransactions: async (req, res) => {
    try {
      ...
      const sliceStart =
        pagination.getPage(page) * pagination.getPerPage(limit) -
        pagination.getPerPage(limit);
      const sliceEnd =
        pagination.getPage(limit) * pagination.getPerPage(limit) - sliceStart;
      ...
      const prevLink = pagination.getPrevLink(
        pagination.getPage(page),
        req.query
      );
      const nextLink = pagination.getNextLink(
        pagination.getPage(page),
        totalPage,
        req.query
      );
  ...
  getTransactionUser: async (req, res) => {
    try {
      ...
      if (findUser) {
        const result = await transactionModel.getTransactionsUser({
          id: parseInt(id),
        });
        res.status(200).send(
          response({
